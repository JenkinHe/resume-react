

export const initialData ={
    TechSkills:[{
        name:'Java',
        image_url:'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS9ZWVEcZU42G4FQbbGPJULwPZYDT67g1kyNLH4liuTIQ&s',
        description:'1st - 4th year uni i progressively learnt it, COMP1000 , COMP2000 etc'
    },
    {
        name:'SQL',
        image_url:'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTfzyAWb-Ulddfff6tYMVr9JGdzS8ycRKQ-7ar0F2WhPg&s',
        description:'Uni database and design course'
    },
    {
        name:'HTML',
        image_url:'data:image/png;base64,',
        description:'Uni 2nd year plus personal studies'
    },
    {
        name:'CSS',
        image_url:'https://1000logos.net/wp-content/uploads/2020/09/CSS-Logo.png',
        description:'Uni 2nd year plus personal studies'
    },
    {
        name:'React',
        image_url:'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSTw3HApB4bsvabXW3L14cV-LhFo0L71QmEESJN3vW9Ow&s',
        description:'java script framework i picked up in my personal time'
    },
    {
        name:'JavaScript',
        image_url:'https://pluralsight2.imgix.net/paths/images/javascript-542e10ea6e.png',
        description:'Uni web tech course 2nd year'
    },
    {
        name:'Flutter',
        image_url:'https://cdn-images-1.medium.com/max/1200/1*5-aoK8IBmXve5whBQM90GA.png',
        description:'3rd year mobile development course'
    },
    {
        name:'Dart',
        image_url:'https://miro.medium.com/v2/resize:fit:1400/1*QCajckOeBhRaLzi0RoFqig.png',
        description:'3rd year mobile development course'
    },
    {
        name:'Tailwind CSS',
        image_url:'https://www.drupal.org/files/project-images/screenshot_361.png',
        description:'something i picked up with react'
    },
    {
        name:'Redux.js',
        image_url:'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT-RG4y6P-3tojx10n0eLj9DKae_4YRyBCNz9WsY0yCuw&s',
        description:'another thing i picked up alongside react'
    },{
        name:'Python',
        image_url:'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQDMh34DPK8RyB1zIg3PGEJtIoH1X5vB1SoIfXYMB23lg&s',
        description:'learnt fundamentals to aleviate boredom but dislike its syntax'
    },
    {
        name:'C',
        image_url:'https://upload.wikimedia.org/wikipedia/commons/thumb/1/18/C_Programming_Language.svg/695px-C_Programming_Language.svg.png',
        description:'Learnt in Uni Systems programming course'
    },{
        name:'AppWrite',
        image_url:'https://avatars.githubusercontent.com/u/25003669?s=280&v=4',
        description:'something i picked up when making a backend for react website'
    },
    {
        name:'MongoDB',
        image_url:'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQyHz-cDTZGdM0svaoQN0PaDuoNyVEAOpj5y1wG3GgtRg&s',
        description:'A database i used when making react website'
    },
    ]
}

